...
final class AuthCommands implements CommandExecutor {
    ...
    private boolean register(Player player, String[] args) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            } catch (SQLException | IllegalArgumentException exception) { plugin.getLogger().warning("Registration database error: " + exception.getMessage()); result(player, "database-error", false); }
        });
        return true;
    }
    private boolean login(Player player, String[] args) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            } catch (SQLException | IllegalArgumentException exception) { plugin.getLogger().warning("Login database error: " + exception.getMessage()); result(player, "database-error", false); }
        });
        return true;
    }
    private boolean changePassword(Player player, String[] args) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            } catch (SQLException | IllegalArgumentException exception) { plugin.getLogger().warning("Password change database error: " + exception.getMessage()); result(player, "database-error", false); }
        });
        return true;
    }
    private boolean valid(Player player, String password) {
        ...
        if (password.getBytes(java.nio.charset.StandardCharsets.UTF_8).length > 72) { player.sendMessage(plugin.locale().message("password-too-long")); return false; }
        return true;
    }
    private boolean begin(Player player) {
        if (inFlight.add(player.getUniqueId())) return true;
        player.sendMessage(plugin.locale().message("auth-processing"));
        return false;
    }
